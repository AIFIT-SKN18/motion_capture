...
def center_zero_pad_or_center_crop(x: np.ndarray, target_T: int) -> np.ndarray:
    ...
    if T < target_T:
        ...
        left = pad_T // 2
        right = pad_T - left
        out = np.zeros((C, target_T, V, M), dtype=x.dtype)
        out[:, left:left+T, :, :] = x
        return out
    else:
        start = (T - target_T) // 2
...
def main():
    ap = argparse.ArgumentParser(description="Single NPY inference (center zero-pad / center crop)")
    ...
    arr = center_zero_pad_or_center_crop(arr, args.window_T)
