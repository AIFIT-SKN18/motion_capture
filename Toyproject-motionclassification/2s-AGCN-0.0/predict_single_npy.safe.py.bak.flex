# ==== HARDEN: fallback defs (safe to duplicate) ====
import numpy as np

import numpy as np

def _strip_leading_batch(x: np.ndarray) -> np.ndarray:
    """허용: (C,T,V,M) / (1,C,T,V,M) / (N,C,T,V,M, N>=1)
    선두 배치 차원이 있으면 0번째 샘플로 자동 축소한다."""
    x = np.asarray(x)
    if x.ndim == 5:
        if x.shape[0] > 1:
            print(f"[WARN] leading batch dim detected: {x.shape[0]} -> picking index 0")
        return x[0]
    return x
...
try:
    ...
    def preprocess_like_train(x, center_joint_idx=1, window_size=300, do_norm=True):
        ...
        # ensure M=2 like training
        if M < 2:
            padm = np.zeros((C, x.shape[1], V, 2 - M), dtype=x.dtype)
...
try:
    ...
    def ensure_ctvm_flex(x, window_size=None, do_norm=None):
        # rely on existing ensure_ctvm in this script
        return ensure_ctvm(x)
# ==== /HARDEN ====

# -*- coding: utf-8 -*-
"""
predict_single_npy.safe.py
...
- Always prints CKPT stats (keys, missing, unexpected)
- CPU/GPU safe (device auto/override, map_location, no .cuda(...) hardcode here)
- Stateless inference guards: requires_grad=False, model.eval(), torch.inference_mode()
- Formatting controls: --precision, --print_all
- Optional fingerprint check to verify parameters didn't change in inference
...
python -u predict_single_npy.safe.py \
  ...
  --npy  "/content/drive/MyDrive/ToyProject/dataset/final_dataset_new/test/joint/sample.npy" \
  --window_T 300 \
  --num_classes 6 \
  --device cpu \
  --apply_other_gate 1 \
...
def ensure_ctvm(arr: np.ndarray) -> np.ndarray:
    x = arr
    if x.ndim == 5 and x.shape[0] == 1:  # (1,C,T,V,M) -> (C,T,V,M)
        x = x[0]
    if x.ndim != 4:
...
def main():
    ...
    if dev.type == "cpu":
        os.environ["CUDA_VISIBLE_DEVICES"] = ""  # avoid accidental CUDA picks in subsystems
    ...
    # ----- load checkpoint (ALWAYS log stats) -----
    ...
    x = preprocess_like_train(ensure_ctvm_flex(np.load(f), window_size=300, do_norm=True))
    x = crop_center_T(x, args.window_T)
    X = torch.from_numpy(x).unsqueeze(0).to(dev)  # [1,C,T,V,M]

    # ----- inference (fully stateless) -----
...
# ==== Injected helpers (preprocess_like_train / ensure_ctvm_flex) ====
import numpy as np
...
# ==== Injected helper functions (ensure_ctvm / preprocess_like_train / ensure_ctvm_flex) ====

def preprocess_like_train(x, center_joint_idx=1, window_size=300, do_norm=True):
    """
    학습 시 Feeder 전처리 근사: 중심관절 원점이동 + 정규화 + 윈도우 맞춤 + M 정렬
    x: (C,T,V,M)
    """
    import numpy as np
    x = np.asarray(x).astype('float32')
...
def ensure_ctvm_flex(x, window_size=None, do_norm=None):
    """
    OVERRIDE: robust 5D handler
      - (1,C,T,V,M) -> squeeze -> ensure_ctvm
      - (2,C,T,V,M) -> joint/bone two-stream
          * env AGCN_MODALITY_IDX: 0=joint, 1=bone, -1=concat to 6ch (model in_channels=6 필요)
      - other 5D (N,C,T,V,M) -> 첫 샘플 사용
      - else -> 기존 ensure_ctvm
    """
    ...
    if x.ndim == 5:
        ...
        # (1,C,T,V,M): squeeze
        if n0 == 1:
            print("[ensure_ctvm_flex][OVR] 1-sample 5D -> squeeze to 4D")
            return ensure_ctvm(x[0])

        # (2,C,T,V,M): two-stream
        if n0 == 2 and c in (3, 6):
            ...
            if mi == -1:
                # concat (2,3,T,V,M) -> (6,T,V,M)
                if c == 3:
                    print("[ensure_ctvm_flex][OVR] concat 2x3ch -> 6ch")
                    return x.reshape(2*3, t, v, m).astype("float32")
                else:
                    print("[ensure_ctvm_flex][OVR][WARN] got (2,6,...), fallback to stream 0")
                    return ensure_ctvm(x[0])

            if mi not in (0, 1):
                print(f"[ensure_ctvm_flex][OVR][WARN] invalid AGCN_MODALITY_IDX={mi_str} -> 0")
                mi = 0

            print(f"[ensure_ctvm_flex][OVR] 2-stream detected -> using stream {mi}")
            return ensure_ctvm(x[mi])

        # other 5D: treat as batch
        print(f"[ensure_ctvm_flex][OVR][WARN] treat 5D as batch (N={n0}) -> first sample")
        return ensure_ctvm(x[0])
...
def ensure_ctvm(x: np.ndarray) -> np.ndarray:
    x = np.asarray(x)
    # 선두 배치 차원 자동 제거
    x = _strip_leading_batch(x)
    # 이제 (C,T,V,M)만 허용
    if x.ndim == 4:
        C, T, V, M = x.shape
        return x
    raise ValueError(f"Expected (C,T,V,M) or (1,C,T,V,M) or (N,C,T,V,M), got {x.shape}")
