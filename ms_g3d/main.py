...
class Processor():
    ...
    def __init__(self, arg):
        ...
        if self.arg.half and APEX_AVAILABLE:
            self.print_log('*************************************')
            self.print_log('*** Using Apex Half Precision Training ***')
            self.print_log('*************************************')
        ...
        elif self.arg.half and not APEX_AVAILABLE:
            self.print_log('*************************************')
            self.print_log('*** Using PyTorch Native AMP ***')
            self.print_log('*************************************')
    ...
    def load_model(self):
        ...
        if self.arg.weights:
            ...
            try:
                ...
                # Only update weights that match in size
                for key, value in weights.items():
                    if key in state and state[key].shape == value.shape:
                        state[key] = value
                    elif key in state:
                        self.print_log(f'Shape mismatch for {key}: skipping ({state[key].shape} vs {value.shape})')
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            if self.arg.half and not APEX_AVAILABLE:
